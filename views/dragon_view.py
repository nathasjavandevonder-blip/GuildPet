...
class DragonView(discord.ui.View):
    ...
    async def shop(self, interaction, button):
        await interaction.response.send_message(embed=make_shop_embed(interaction.guild.id), view=ShopView(), ephemeral=True)
    ...
    async def leaderboard(self, interaction, button):
        ...
        if not rows:
            await interaction.response.send_message("No leaderboard yet.", ephemeral=True); return
        lines = []
    ...
    async def memories(self, interaction, button):
        ...
        if not rows:
            await interaction.response.send_message("📖 No dragon memories yet.", ephemeral=True); return
