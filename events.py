...
def create_event_record(guild_id: int, event_type: str, message_id: int):
    ...
    cur.execute("INSERT OR REPLACE INTO events (guild_id, event_type, expires_at, claimed_by, message_id) VALUES (?, ?, ?, NULL, ?)", (guild_id, event_type, expires.isoformat(), message_id))
...
def claim_event(guild_id: int, user):
    ...
    event = cur.fetchone()
    if not event:
        con.close(); return False, "This event is no longer active."
    if event["claimed_by"]:
        con.close(); return False, "Someone already claimed this event."
...
def random_event_embed():
    ...
    embed = discord.Embed(title=title, description=text + "\n\nFirst person to claim gets a reward.\nThis event expires in **15 minutes**.", color=0x7B2CFF)
