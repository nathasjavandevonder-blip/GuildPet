...
def mood_from_stats(d):
    if d["hunger"] < 25: return "Hungry"
    if d["cleanliness"] < 25: return "Messy"
    if d["energy"] < 25: return "Sleepy"
    if d["bond"] >= 80 and d["happiness"] >= 80: return "Affectionate"
    if d["happiness"] >= 80: return "Happy"
    return "Curious"

def time_based_message():
    hour = datetime.now(timezone.utc).hour
    if 5 <= hour < 11: return random.choice(DRAGON_MESSAGES["morning"])
    if hour >= 22 or hour < 5: return random.choice(DRAGON_MESSAGES["night"])
    return None

def pick_idle_message(d):
    mood = mood_from_stats(d)
    timed = time_based_message()
    if timed and random.random() < 0.35: return timed
    if mood == "Hungry": return random.choice(DRAGON_MESSAGES["hungry"])
    if mood == "Sleepy": return random.choice(DRAGON_MESSAGES["sleepy"])
    if mood == "Messy": return random.choice(DRAGON_MESSAGES["messy"])
    if mood == "Affectionate": return random.choice(DRAGON_MESSAGES["affectionate"])
...
def check_cooldown(guild_id: int, user_id: int, action: str, minutes: int = 30):
    ...
    row = cur.fetchone(); now = datetime.now(timezone.utc)
    if row:
...
def update_daily_streak(guild_id: int, user_id: int):
    ...
    p = cur.fetchone()
    if not p:
        con.close(); return 0, False
    if p["last_daily"] == today:
        con.close(); return p["streak"], False
    if p["last_daily"]:
        try:
            last = date.fromisoformat(p["last_daily"]); now_date = date.fromisoformat(today)
...
def add_player_reward(guild_id: int, user_id: int, tokens: int, points: int, action: str):
    ...
    con = connect(); cur = con.cursor()
    cur.execute("INSERT INTO players (guild_id, user_id, tokens, points) VALUES (?, ?, ?, ?) ON CONFLICT(guild_id, user_id) DO UPDATE SET tokens = tokens + excluded.tokens, points = points + excluded.points", (guild_id, user_id, tokens, points))
    if column:
...
def apply_action(guild_id: int, user, action: str):
    ...
    con = connect(); cur = con.cursor()
    cur.execute("UPDATE dragon SET hunger=?, happiness=?, energy=?, cleanliness=?, bond=?, xp=?, guild_tokens=?, lifetime_guild_tokens=?, pose=?, visual_event=?, last_action_text=?, dragon_message=? WHERE guild_id=?", (hunger, happiness, energy, cleanliness, bond, xp, guild_tokens, lifetime_guild_tokens, e["pose"], "None", text, dragon_message, guild_id))
    con.commit(); con.close()
...
def decay_dragon(guild_id: int):
    d = get_dragon(guild_id); message = pick_idle_message(d); mood = mood_from_stats(d)
    weather_options = ["Clear", "Clear", "Clear", "Rain", "Stars"]
    if datetime.now(timezone.utc).month in [12, 1, 2]: weather_options.append("Snow")
    con = connect(); cur = con.cursor()
    cur.execute("UPDATE dragon SET hunger=?, happiness=?, energy=?, cleanliness=?, pose=?, mood=?, weather=?, visual_event=?, dragon_message=?, last_decay=? WHERE guild_id=?", (clamp(d["hunger"] - 3), clamp(d["happiness"] - 2), clamp(d["energy"] - 2), clamp(d["cleanliness"] - 2), random.choice(["waiting", "sleeping", "looking around", "stretching", "guarding the lair"]), mood, random.choice(weather_options), "None", message, datetime.now(timezone.utc).isoformat(), guild_id))
    con.commit(); con.close()
    update_personality(guild_id)
