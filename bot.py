...
from embeds import make_dragon_embed, make_profile_embed, make_art_status_embed, make_needed_images_embed
...
async def set_dragon_field(interaction, field, value, msg, memory=None):
    ...
    cur.execute(f"UPDATE dragon SET {field}=?, dragon_message=?, last_action_text=? WHERE guild_id=?", (value, msg, msg, interaction.guild.id))
    con.commit(); con.close()
    if memory: add_memory(interaction.guild.id, memory)
...
async def dragon_setup(interaction: discord.Interaction):
    ...
    msg = await interaction.channel.send(embed=embed, file=file, view=DragonView()) if file else await interaction.channel.send(embed=embed, view=DragonView())
    con = connect(); cur = con.cursor()
...
async def dragon_name(interaction: discord.Interaction, name: str):
    if len(name) > 32:
        await interaction.response.send_message("Name is too long. Max 32 characters.", ephemeral=True); return
    await set_dragon_field(interaction, "dragon_name", name, f"🐉 My name is {name} now!", f"The guild named the dragon {name}.")
...
async def dragon_color(interaction: discord.Interaction, color: str):
    if len(color) > 24:
        await interaction.response.send_message("Color name is too long. Max 24 characters.", ephemeral=True); return
    await set_dragon_field(interaction, "dragon_color", color, f"🎨 I feel like a {color} dragon.", f"The dragon became a {color} dragon.")
...
async def dragon_accessory(interaction: discord.Interaction, accessory: str):
    if len(accessory) > 32:
        await interaction.response.send_message("Accessory name is too long. Max 32 characters.", ephemeral=True); return
    await set_dragon_field(interaction, "accessory", accessory, f"🎩 Do you like my {accessory}?", f"The dragon received an accessory: {accessory}.")
...
async def dragon_weather(interaction: discord.Interaction, weather: str):
    if len(weather) > 24:
        await interaction.response.send_message("Weather name is too long. Max 24 characters.", ephemeral=True); return
    await set_dragon_field(interaction, "weather", weather, f"🌤️ The weather is {weather} around the lair.")
...
async def dragon_add_tokens(interaction: discord.Interaction, amount: int):
    con = connect(); cur = con.cursor()
    cur.execute("UPDATE dragon SET guild_tokens=guild_tokens+?, dragon_message=?, last_action_text=? WHERE guild_id=?", (amount, "The guild treasure pile grew!", f"🪙 Added {amount} Guild Tokens for testing.", interaction.guild.id))
    con.commit(); con.close()
    await update_dragon_message(interaction.guild)
...
async def dragon_art_status(interaction: discord.Interaction):
    await interaction.response.send_message(embed=make_art_status_embed(interaction.guild.id), ephemeral=True)
...
async def dragon_title(interaction: discord.Interaction, title: str):
    if len(title) > 32:
        await interaction.response.send_message("Title is too long. Max 32 characters.", ephemeral=True); return
    con = connect(); cur = con.cursor()
    cur.execute("INSERT INTO players (guild_id, user_id, keeper_title) VALUES (?, ?, ?) ON CONFLICT(guild_id, user_id) DO UPDATE SET keeper_title=excluded.keeper_title", (interaction.guild.id, interaction.user.id, title))
    con.commit(); con.close()
...
async def dragon_streak(interaction: discord.Interaction, member: discord.Member = None):
    ...
    if not p:
        await interaction.response.send_message(f"{member.display_name} has no streak yet.", ephemeral=True); return
    await interaction.response.send_message(f"🔥 **{member.display_name}'s Keeper Streak**\nCurrent: **{p['streak']} days**\nBest: **{p['best_streak']} days**\nLast care day: **{p['last_daily'] or 'Never'}**", ephemeral=True)
