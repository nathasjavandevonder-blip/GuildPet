...
STAGE_THRESHOLDS = [
    ("Egg", 0),
    ("Hatchling", 1000 * XP_MULTIPLIER),
    ("Young Dragon", 5000 * XP_MULTIPLIER),
    ("Adult Dragon", 15000 * XP_MULTIPLIER),
    ("Ancient Dragon", 40000 * XP_MULTIPLIER),
    ("Elder Dragon", 100000 * XP_MULTIPLIER),
]
...
def get_stage(xp: int):
    if xp >= 100000 * XP_MULTIPLIER:
        return "Elder Dragon", "🐉"
    if xp >= 40000 * XP_MULTIPLIER:
        return "Ancient Dragon", "🐉"
    if xp >= 15000 * XP_MULTIPLIER:
        return "Adult Dragon", "🐉"
    if xp >= 5000 * XP_MULTIPLIER:
        return "Young Dragon", "🐲"
    if xp >= 1000 * XP_MULTIPLIER:
        return "Hatchling", "🐲"
    return "Egg", "🥚"
...
def next_stage_info(xp: int):
    current_xp = 0
    next_name = "Hatchling"
    next_xp = 1000 * XP_MULTIPLIER

    for i, (name, needed) in enumerate(STAGE_THRESHOLDS):
        if xp >= needed:
            current_xp = needed
            if i + 1 < len(STAGE_THRESHOLDS):
                next_name, next_xp = STAGE_THRESHOLDS[i + 1]
            else:
...
def detailed_stage_title(xp: int):
    # Extra visible growth before the first hatch, scaled with XP_MULTIPLIER.
    if xp < 250 * XP_MULTIPLIER:
        return "Tiny Egg", "🥚"
    if xp < 500 * XP_MULTIPLIER:
        return "Large Egg", "🥚"
    if xp < 750 * XP_MULTIPLIER:
        return "Cracked Egg", "🥚"
    if xp < 1000 * XP_MULTIPLIER:
